<?php	
	...
	if($_SERVER['REQUEST_METHOD']=="POST")
	{
		if(isset($_POST['area_lat'])  && isset($_POST['area_lan']) && isset($_POST['cat_id']) && isset($_POST['area_id']))
		{
			...
			$query = "SELECT * FROM agency WHERE  agency_lat BETWEEN '".$minLat."' AND '".$maxLat."' AND agency_long BETWEEN '".$minLng."' AND '".$maxLng."'  AND `agency_active`= 0 And agency_id in (select agency_id from agency_category where cat_id=".$cat_id." and agency_id in(select agency_id from area_by_agency where area_id=".$area_id."))";	
