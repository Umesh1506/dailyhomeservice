...
{
	if(isset($_POST['user_id']) && isset($_POST['message']) && isset($_POST['title']))
	{
			$uid=$_POST['user_id'];
			$title =  $_POST['title'];
			$message =$_POST['message'];
		
			$query = "select user_fname,user_devicetoken from user where user_id = ".$uid;
			$result = mysqli_query($conn, $query);
			$row = mysqli_fetch_assoc($result);
			$user_devicetoken =$row['user_devicetoken'];
